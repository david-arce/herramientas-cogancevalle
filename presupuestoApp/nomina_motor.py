...
def filas_de_conceptos(c, ctx):
    ...
    if carga.get('agrupar'):
        ...
        registros = consulta.values(*ident, 'nombre_con', 'concepto_f', *historico).order_by('pk')
    ...
    return distribuir_filas(c, filas, repartos(), p)
...
def distribuir_filas(c, filas, reparto_por_cedula, p, usuario=''):
...
def aplicar_distribucion(cedulas, usuario=''):
    ...
    with bloqueo():
        for c in BASE:
            ...
            if not filas:
                continue
            nuevas = distribuir_filas(c, filas, por_cedula, ctx.p, usuario)
...
def regenerar(ctx, c):
    ...
    vistas = Counter()
    nuevas = []
    for datos in GENERADORES[c.slug](ctx, c):
...
def guardar(slug, filas_json, usuario=''):
    ...
    with bloqueo():
        ...
        campos = [*CAMPOS_TEXTO, 'base', 'factor', 'historico', *MESES, 'total',
                  'origen', 'actualizado_por']
...
CAMPOS_SALIDA = ('id', 'tipo', *CAMPOS_TEXTO, 'base', 'factor', 'historico',
